...
while True:
    ...
    if islem == "1":
        ...
        tag = kullanici.isim
        if tag in kullanici_sozluğu.keys():
            print("Böyle bir kullanıcı vardır...")
            time.sleep(2)
            continue
        else:
            kullanici_sozluğu[tag] = kullanici
            print(f"{kullanici_sozluğu[tag].isim} kullancı {kullanici_sozluğu[tag].ana_para} bütçesiyle kayıt oluşturulmuştur.")
            time.sleep(2)
        ...
        print(f"{tag.title()} ketegorisi olan ödeme {g2[1]} Türk lirası ödeme yapılmıştır. Yaptığınız alışveriş, provizyonda beklemektedir. Provizyondan çıkması için -- işlem == 3'den doğrulama yapmanız gerekmektedir....")
        time.sleep(2)
        ...
        isim = isim.lower()
        odeme_yontemi = odeme_yontemi.lower()
        ...
        path2 = f"./{kullanici_nesnesi.isim}-detail.txt"

        if os.path.isfile(path2):
            with open(f'{kullanici_nesnesi.isim}-detail.txt', 'ta+', encoding='utf-8') as fa:
                print("-" * 20, "-" * 20, "-" * 20, file=fa)
                for i in kullanici_nesnesi.odemeler:
                    print(f"{i.odeme_turu}\t", f"{i.fiyati}", file=fa)
                print("Toplam Fiyat\t", total, file=fa)

        else:
            with open(f'{kullanici_nesnesi.isim}-detail.txt', 'tw', encoding='utf-8') as fa:
                print("Ödeme Türü","Fiyatı",file=fa)
                for i in kullanici_nesnesi.odemeler:
                    print(f"{i.odeme_turu}\t", f"{i.fiyati}", file=fa)
                print("Toplam\t", total, file=fa)
        ...
        try:
            ...
            if isim in kullanici_sozluğu.keys():
                ...
                print(
                    f"{kullanici_nesnesi.isim} kullacının ödeme yaptıktan sonra kalan parası {kullanici_nesnesi.ana_para}")
                time.sleep(2)
                os.remove(path1)
