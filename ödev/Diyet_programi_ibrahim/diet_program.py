...
class Nutritionist():
    ...
    def start(self):
        print(
            "Hoşgeldiniz Girdiğiniz yemekler 100 gram üzerinden kalori hesabı yapılacaktır. Çıktılar ise grama karşlık gelen kalorileri gösterecektir...\n choise == 7'e basarsanız Hazır yemekler sisteme eklenecektir. Diğer durumda kendiniz ekleyebilirsiniz.")
        choise = input(
            "1-Kullanıcı oluştur\n2-Gıda ürünü ekle\n3-Beş günlük yemek listesini göster\n4-Kayıtlı olan kullanıcıları göster\n5-Kayıtlı olan Besinleri Göster\nİşlem seçiniz: ")
        if choise == "1":
            self.create_user()
            time.sleep(3)
            self.start()
